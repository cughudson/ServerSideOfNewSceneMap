...
public class FileController extends BaseController {
  ...
  public GenericResponse upload(MultipartFile file) throws IOException {
    ...
    try {
      ...
      String url=imgFolder + destUrl + destFileName;
      processImg(32, destPath, fileId, newFile, suffix, destUrl, result,url);
      processImg(300, destPath, fileId, newFile, suffix, destUrl, result,url);
      processImg(2048, destPath, fileId, newFile, suffix, destUrl, result,url);
      result.put("url", url);
  ...
  private void processImg(int maxWidth, String destPath, String fileId, File newFile, String suffix, String destUrl, JSONObject result,String url) throws IOException {
    ...
    if(imageWidth >maxWidth){
      try {
        ...
        newUrl=imgFolder + destUrl + fileId + "_" + maxWidth + suffix;
      } catch (Exception e) {
        logger.error(e.getMessage(), e);
      }
    }
    result.put("url_" + maxWidth,newUrl);
  }
